...
#
# Copy of ArangoDB entrypoint with options --server.endpoint=unix:///tmp/arangodb-tmp.sock,
# --server.authentication false added to arangosh call for init db scripts.
#
set -eo pipefail
	...
	if [ ! -f /var/lib/arangodb3/SERVER ]; then
		...
		for f in /docker-entrypoint-initdb.d/*; do
			case "$f" in
				...
				*.js)     echo "$0: running $f"; arangosh --server.endpoint=unix:///tmp/arangodb-tmp.sock --server.authentication false --javascript.execute "$f" ;;
